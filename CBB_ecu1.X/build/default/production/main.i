...
const unsigned char password[4] = {11, 12, 11, 12};

unsigned char pwd_buf[4];
...
void screen_default(){
        sec = bcd_to_dec(read_ds1307(0x00) & 0x7F);
        min = bcd_to_dec(read_ds1307(0x01));
        hour = bcd_to_dec(read_ds1307(0x02) & 0x3F);
        Event e;
...
void screen_menu(){
    clcd_write(0x01, 0);
    clcd_print(menu==0?">VIEW LOG":" VIEW LOG", (0x80 + (0)));
    clcd_print(menu==1?">SET TIME":" SET TIME", (0xC0 + (0)));
}
...
void main(void){
    ...
    while (1){
        ...
        if (is_collision == 1)
        {
            logging_enabled = 0;
            clcd_write(0x01, 0);
            clcd_print("COLLISION HAPPENED", (0x80 + (0)));
            clcd_print("ENGINE OFF", (0xC0 + (0)));
            continue;
        }
        else{
            ...
            switch(state){
                ...
                case ST_LOGIN:
                    if (login_locked){
                        clcd_print("LOGIN LOCKED", (0x80 + (0)));
                        clcd_print("WAIT 3 MIN", (0xC0 + (0)));
                    ...
                    break;
                case ST_MENU:
                    if(key == 1 && menu > 0) menu--;
                    else if(key == 2 && menu < 1) menu++;
                    else if(key == 3){
                        clcd_write(0x01, 0);
                ...
                case ST_VIEW_LOG:
                    if(key == 1){
                        event_idx++;
                        if(event_idx >= 10) event_idx = 0;
                    }
                    else if(key == 2){
                        if(event_idx == 0) event_idx = 10 - 1;
                        else event_idx--;
                    }
                    else if(key == 3){
                            clcd_write(0x01, 0);
